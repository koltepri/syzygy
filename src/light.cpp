...
//functions
static bool PointInsideObject(Vector2 point, const Obstacle& obstacle){ //static = private
...
static Vector2 NormVector(Vector2 point, const Obstacle& object){
    if (object.type == CIRCLE) {
        ...
        return { ab.x/c, ab.y/c };} //normalized dx dy stefnur
    ...
    if (object.type == KASSI) {
        ...
        if (nearest == right)return { 1, 0};
        if (nearest == top)  return { 0,-1}; //raylib https://i1.sndcdn.com/artworks-000394759668-wan0rk-t1080x1080.jpg
        else                 return { 0, 1};}
    return { 0, 0 };}

Light::Light(int posX, int posY)
    : x(posX)
    , y(posY)
    , RayCount(3600){}
...
void Light::Draw(const Obstacle* obstacles, int count) const
{
    ...
    for (int i = 0; i < RayCount; i++) {
        ...
        while (true) {
            ...
            if (Walking_x < 0 || Walking_x > ScreenWidth || Walking_y < 0 || Walking_y > ScreenHeight) {
                DrawLine(StartX, StartY, Walking_x, Walking_y, Fade(YELLOW, 0.3f));
                break;}
            if (sqrtf((Walking_x - x)*(Walking_x - x) + (Walking_y - y)*(Walking_y - y)) > 600) {
                DrawLine(StartX, StartY, Walking_x, Walking_y, Fade(YELLOW, 0.3f));
                break;}
